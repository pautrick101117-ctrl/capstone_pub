    ...
    <section className="section-shell py-10 sm:py-14">
      ...
      {isNews ? (
        <div className="mt-8 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {loading ? Array.from({ length: 6 }, (_, index) => <Card key={index} className="space-y-4"><Skeleton className="h-48 w-full" /><Skeleton className="h-5 w-2/3" /><Skeleton className="h-4 w-24" /><Skeleton className="h-16 w-full" /></Card>) : items.map((item) => <button key={item.id} type="button" onClick={() => setSelected(item)} className="text-left"><Card className="h-full overflow-hidden p-0 hover:-translate-y-1 hover:border-[var(--brand-200)] hover:shadow-md"><div className="h-48 bg-[var(--brand-50)]">{item.imageUrl ? <img src={item.imageUrl} alt={item.title} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center bg-gradient-to-br from-[var(--brand-100)] to-white text-[var(--brand-600)]"><Newspaper className="h-9 w-9" /></div>}</div><div className="space-y-3 p-5"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--brand-500)]">{formatDate(item.createdAt || item.created_at)}</p><h2 className="text-xl font-bold text-[var(--brand-900)]">{item.title}</h2><p className="line-clamp-4 text-sm leading-6 text-stone-600">{item.body}</p></div></Card></button>)}
        </div>
      ) : (
